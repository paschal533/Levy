...
const SafeDetails = () => {
        ...
        const getPendingTransactions = async () => {
                try{
                    if(conversactionAddress){
                        ...
                        const pendingTransactions = await safeService.getPendingTransactions(conversactionAddress)
    ...
    const deposit = async () => {
        ...
        try{
         const safeAmount = ethers.utils.parseUnits(ethAmount, 'ether').toHexString()
    ...
    const CreateTransaction = async () => {
        ...
        try{
            const amount = ethers.utils.parseUnits(transactionAmount, 'ether').toString()
            const RPC_URL='https://eth-goerli.public.blastapi.io'
            const provider = new ethers.providers.JsonRpcProvider(RPC_URL)
            const owner1Signer = new ethers.Wallet(privateKey, provider)
            const txServiceUrl = 'https://safe-transaction-goerli.safe.global'
    ...
    return(
        ...
        >
            ...
            <Flex
                ...
                flexGrow={1}
            >
                <Text fontSize={25} fontWeight="bold">Safe account balance</Text>
                <Text fontSize={35} fontWeight="medium">$4,500</Text>
                <Flex>
                    <div className="p-2 w-[150px] cursor-pointer hover:bg-[#a59999] justify-center text-center bg-[#828181] rounded-2xl" onClick={() => {onOpen(), setContent("send")}}>Send token</div>
                ...
                <Flex width="100%" mt={3} justifyContent="space-around" justifyItems="center">
                    <Flex>
                ...
                <Flex width="100%" mt={4} justifyContent="space-around" justifyItems="center">
                    <Flex>
                ...
                <Button onClick={deposit} isLoading={depositing}  colorScheme='teal' className="bg-[#2d89e6]"  size='md' color="white">Deposit</Button>
            </Flex>
            <Drawer
                isOpen={isOpen}
                placement='right'
                ...
                <DrawerOverlay />
                 {content === "send" ? (
                    ...
                    <DrawerBody>
                        ...
                        <br />
                        <Input className="mt-4" placeholder='Ether Amount' value={transactionAmount} onChange={(e) => setTransactionAmount(e.target.value)} />
                    </DrawerBody>
                    ...
                    <DrawerFooter>
                        ...
                        <Button onClick={CreateTransaction} colorScheme='blue'>Send</Button>
                    </DrawerFooter>
                    </DrawerContent>
                 ) : (
                    <DrawerContent>
